...
test('throws exception when seller information is invalid', function () {
    ...
    $invoice->sellerInformation()
        ...
        ->setTin('123'); // Should be more than 6 digits

    expect(fn () => $invoice->generateXml())
        ->toThrow(InvalidArgumentException::class, 'Invalid TIN format. Must be more than 6 digits');
});
...
<cac:InvoiceLine>
    ...
    <cac:Price>
        <cbc:PriceAmount currencyID="JO">10.000000000</cbc:PriceAmount>
        <cac:AllowanceCharge>
...
test('it auto-calculates invoice totals correctly', function () {
    ...
    expect($xml)->toContain('<cac:TaxTotal>')
        ->and($xml)->toContain('<cbc:TaxAmount currencyID="JO">16.000000000</cbc:TaxAmount>')
        ->and($xml)->toContain('<cbc:LineExtensionAmount currencyID="JO">100.000000000</cbc:LineExtensionAmount>')
        ...
        ->and($xml)->toContain('<cbc:TaxExclusiveAmount currencyID="JO">100.000000000</cbc:TaxExclusiveAmount>')
        ->and($xml)->toContain('<cbc:TaxInclusiveAmount currencyID="JO">116.000000000</cbc:TaxInclusiveAmount>')
        ->and($xml)->toContain('<cbc:PayableAmount currencyID="JO">116.000000000</cbc:PayableAmount>');
});
