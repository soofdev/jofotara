...
class InvoiceLineItem implements ValidatableSection
{
    ...
    public function toXml(): string
    {
        ...
        $xml[] = sprintf('        <cbc:RoundingAmount currencyID="JOD">%.2f</cbc:RoundingAmount>', $taxInclusiveAmount);
        $xml[] = '        <cac:TaxSubtotal>';
        $xml[] = sprintf('            <cbc:TaxAmount currencyID="JOD">%.2f</cbc:TaxAmount>', $taxAmount);
        $xml[] = '            <cac:TaxCategory>';
        ...
        // Price information
        $xml[] = '    <cac:Price>';
        $xml[] = sprintf('        <cbc:PriceAmount currencyID="JOD">%.2f</cbc:PriceAmount>', $this->unitPrice);
        ...
        $xml[] = sprintf('            <cbc:Amount currencyID="JOD">%.2f</cbc:Amount>', $this->discount);
        $xml[] = '        </cac:AllowanceCharge>';
        $xml[] = '    </cac:Price>';
        $xml[] = '</cac:InvoiceLine>';
